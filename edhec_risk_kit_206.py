...
def cppi1(returns, cppi=100, floor_percent=0.7, m=6, safe_asset_rate=0.02):
...
def plot_cppi(returns):
    returns =returns
    cppi, floor_values = cppi1(returns)
...
def tipp(returns, cppi=100, floor_percent=0.7, m=2, safe_asset_rate=0.02, gap=1):
    safe_assets = pd.DataFrame().reindex_like(returns)
    safe_assets[:] = safe_asset_rate / 12
...
def plot_tipp(returns):
    returns = returns
    tipp, floor_values = tipp(returns, m=4, gap=1)
    normal = 100*(1+returns).cumprod()
